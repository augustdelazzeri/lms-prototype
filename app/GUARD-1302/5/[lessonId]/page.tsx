...
import CoursePlayerHeader from "@/components/learner/CoursePlayerHeader-guard-1302";
import CoursePlayerSidebar from "@/components/learner/CoursePlayerSidebar-guard-1302";
import LessonContentRenderer from "@/components/learner/LessonContentRenderer-guard-1302";
...
export default function CoursePlayerLessonPage() {
  ...
  useEffect(() => {
    ...
    if (!courseData) {
      router.push("/GUARD-1302/learner");
      return;
    }
    ...
    if (!lesson) {
      ...
      if (progressCourse?.lastLessonId) {
        router.replace(`/GUARD-1302/learner/courses/${courseId}/lessons/${progressCourse.lastLessonId}`);
      } else if (lessonsData.length > 0) {
        router.replace(`/GUARD-1302/learner/courses/${courseId}/lessons/${lessonsData[0].id}`);
      } else {
        router.push("/GUARD-1302/learner");
      }
      return;
    ...
    if (!isLessonUnlocked(courseData, lessonId, getCurrentUser().id)) {
      ...
      if (unlockedLesson) {
        router.replace(`/GUARD-1302/learner/courses/${courseId}/lessons/${unlockedLesson.id}`);
        return;
      }
    }
  ...
  const handleLessonClick = useCallback(
    (newLessonId: string) => {
      ...
      router.push(`/GUARD-1302/learner/courses/${courseId}/lessons/${newLessonId}`);
    },
        ...
        <div className="flex flex-col h-screen bg-white">
          <div className="bg-white border-b border-gray-200 px-6 py-2">
            <div className="max-w-full mx-auto flex items-center gap-2 text-sm text-gray-600">
              <Link href="/GUARD-1302/learner" className="hover:text-gray-900">Back to Courses</Link>
              <span>/</span>
      ...
      <div className={`flex flex-col h-screen ${highContrast ? "bg-white" : "bg-gray-50"}`}>
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-2">
          <div className="max-w-full mx-auto flex items-center gap-2 text-sm text-gray-600">
            <Link href="/GUARD-1302/learner" className="hover:text-gray-900">Back to Courses</Link>
            <span>/</span>
            <Link href={`/GUARD-1302/learner/courses/${courseId}`} className="hover:text-gray-900">{course.title}</Link>
            <span>/</span>
